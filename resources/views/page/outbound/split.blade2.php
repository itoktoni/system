...
<div class="row">
    ...
    {!! Form::open(['route' => $form.'_split', 'class' => 'form-horizontal', 'files' => true]) !!}  
    <div class="panel panel-default">
        <header class="panel-heading">
            <h2 class="panel-title">Batch Split Qty</h2>
        </header>
        <div class="panel-body">
            <div class="col-md-12 col-lg-12">
                <div class="form-group">
                    ...
                    <div class="col-md-5 {{ $errors->has('product_id') ? 'has-error' : ''}}">
                        <select class="form-control col-md-4 option" name="product">
                            ...
                            <option @isset($data) {{ $value->product_id == $data->product_id ? 'selected="selected"' : '' }} @endisset value="{{ $value->barcode }}">{{ $value->product_name }} ( {{$value->barcode}} )</option>
                            @endforeach
                        </select>
                    </div> 
                    <div>
                ...
                <div class="form-group">
                    ...
                    <div>
                        @isset($data)
                        <a target="_blank" href="{!! route("{$form}_split", ["print" => $data->barcode ]) !!}" class="btn btn-danger">Print Barcode Split
                        </a>
                        @endisset
                    </div>
                </div>
