...
class GroupUserController extends Controller
{
    ...
    public function update()
    {
        $id = request()->get('code');
        if (!empty($id)) {
            $getData = $this->validasi($this->model->baca($id));
            $group_module      = new \App\GroupModule();
            $list_group_module = $group_module->getGroupByUser($id)->get();

            $user      = new \App\User();
            $list_user = $user->getUserByGroupUser($id)->get();
            ...
            if (request()->isMethod('POST')) {
                ...
                if (request()->exists('user_list')) {
                    $this->model->saveUser($id, request()->get('user_list'));
                }
                if (request()->exists('group_module')) {
                    $this->model->saveKoneksiModule($id, request()->get('group_module'));
                }
